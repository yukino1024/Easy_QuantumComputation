...
class wri_excel:
    ...
    def add_data(self, i,ras_data,pt2_data,osc_data,occ_data):
        if osc_data == '':
            if pt2_data == 'Empty':
                self.data[str(i)] = [str(i),float(ras_data),'=(B'+str(i+1)+'-B2)*627.5',pt2_data,'Empty','Empty',osc_data,occ_data]
            else:
                self.data[str(i)] = [str(i),float(ras_data),'=(B'+str(i+1)+'-B2)*627.5',float(pt2_data),'=(D'+str(i+1)+'-D2)*627.5','=28591/'+'E'+str(i+1),osc_data,occ_data]
        else:
            if pt2_data == 'Empty':
                self.data[str(i)] = [str(i),float(ras_data),'=(B'+str(i+1)+'-B2)*627.5',pt2_data,'Empty','Empty',float(osc_data),occ_data]
            else:
                self.data[str(i)] = [str(i),float(ras_data),'=(B'+str(i+1)+'-B2)*627.5',float(pt2_data),'=(D'+str(i+1)+'-D2)*627.5','=28591/'+'E'+str(i+1),float(osc_data),occ_data]
